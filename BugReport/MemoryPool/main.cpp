...
int main(int argc, char **argv) {
    ...
    systemSP[0].pos.x = 1.5;
    systemSP[0].pos.y = 1.5;
    systemSP[0].pos.z = 1.5;
    systemSP[0].gid = 0;
    systemSP[0].RSearch = 0.5; // EPI Rsearch = 1.0, EPJ Rsearch = 1.0
    systemSP[0].nb.clear();
    systemSP[1].pos.x = 2.5;
    systemSP[1].pos.y = 2.5;
    systemSP[1].pos.z = 2.45;
    systemSP[1].gid = 1;
    systemSP[1].RSearch = 0.5; // EPI Rsearch = 10.0, EPJ Rsearch = 10.0
    systemSP[1].nb.clear();

    dinfo.initialize(0.5);
    dinfo.setBoundaryCondition(PS::BOUNDARY_CONDITION_PERIODIC_X); // PERIODIC_X
    dinfo.setPosRootDomain(PS::F64vec3(0, 0, 0), PS::F64vec3(5, 5, 5));
    ...
    std::cout << "n_seg before treeSymmetry" << PS::MemoryPool::getNSegment() << std::endl;
    treeSymmetry.calcForceAllAndWriteBack(calcNearForceFtr, systemSP, dinfo);
    std::cout << "n_seg after treeSymmetry" << PS::MemoryPool::getNSegment() << std::endl;
    ...
    std::cout << "n_seg before treeScatter" << PS::MemoryPool::getNSegment() << std::endl;
    treeScatter.calcForceAllAndWriteBack(calcNearForceFtr, systemSP, dinfo);
    std::cout << "n_seg after treeScatter" << PS::MemoryPool::getNSegment() << std::endl;
